...
// --- NEW CODE SECTION ---
// Load your logo image so we can use it for the board background.
var boardLogo = new Image();
boardLogo.src = 'img/my-logo.png';

// This function will run automatically as soon as the image is finished loading.
// This is a robust way to ensure the board is redrawn with the logo once it's ready.
boardLogo.onload = function() {
	// Redraw the board with the now-loaded image.
	drawBoard();
};
...
function drawBoard(alternate) { 
	...
	// Check if the logo is loaded and ready before drawing it
	if (boardLogo.complete && boardLogo.naturalHeight !== 0) {
		// Draw the logo in the center of the board, but larger.
		// The coordinates are for the ghost house area.
		ctx.drawImage(boardLogo, 235, 245, 80, 80);
	}
	// --- END OF UPDATED CODE SECTION ---
}
