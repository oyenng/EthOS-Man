...
// --- NEW CODE SECTION ---
// Load your logo image so we can use it for the lives counter.
var pacmanLogo = new Image();
pacmanLogo.src = 'img/my-logo.png';

// This function will run automatically as soon as the image is finished loading.
pacmanLogo.onload = function() {
	// This redraws the lives counter with the now-loaded image.
	lifes();
};
...
function lifes(l) { 
	...
	// Increase canvas size to accommodate larger logos
	...
	if (canvas.getContext) { 
		...
		// Check if the logo image is loaded and ready before trying to draw it
		if (pacmanLogo.complete && pacmanLogo.naturalHeight !== 0) {
			// Loop to draw one logo for each life remaining
			// I have increased the width and height to 45 to make the logo much larger.
			for (var i = 0, imax = LIFES; (i < imax && i < 4); i++) { 
				ctx.drawImage(pacmanLogo, 5 + (i * 50), 2, 45, 45); 
			}
		}
	}
}
